...
https://devpulse.thingz.io is a reference deployment run by the maintainer. A report against the
...
- Privilege escalation to the admin surface, or any admin-only capability
  reachable without admin.
...
- Injection reachable through repository data, GitHub API responses, or any
  other input the importer ingests — including into prompts sent to the
  configured LLM, where the output is then trusted.
- Secrets or tokens leaking into logs, error messages, digest emails, or
  exported data.
