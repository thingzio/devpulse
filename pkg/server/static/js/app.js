...
function loadRepoOverview(url) {
    ...
    $.get(url, function (resp) {
        ...
        $.each(data, function (i, r) {
            ...
            var $weeklyCell = $('<td class="num hide-mobile"></td>').text(r.weekly_events.toLocaleString() + ' (' + wpct + '%)');
            if (wcolor) $weeklyCell.css('color', wcolor);
            $row.append($weeklyCell);

            var scoredPct = r.contributors > 0 ? Math.round(r.scored / r.contributors * 100) : 0;
            $row.append($('<td class="num hide-mobile"></td>').text(r.contributors.toLocaleString() + ' (' + scoredPct + '%)'));
...
function showEntityDevelopers(entity) {
    ...
    $.get(`/data/entity/developers?e=${encodeURIComponent(entity)}`, function (data) {
        ...
        list.append(`<li class="entity-popover-hint">Wrong affiliation? Update the company field in your <a href="https://github.com/settings/profile" target="_blank">GitHub profile</a>.</li>`);
    }).fail(function () {
