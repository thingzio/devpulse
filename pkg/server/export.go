...
func exportRepoCSVs(ctx context.Context, zw *zip.Writer, s data.Store, org, repo string, months int, prefix string) {
	...
	// Summary
	if summary, err := s.GetInsightsSummary(ctx, o, r, nil, months); err == nil && summary != nil {
	...
	if events, err := s.SearchEvents(ctx, criteria); err == nil && len(events) > 0 {
		rows := [][]string{{"date", "type", "username", "org", "repo", "url"}}
	...
	// Developers
	if devs, err := s.GetDeveloperPercentages(ctx, nil, o, r, nil, months); err == nil && len(devs) > 0 {
		rows := [][]string{{"developer", "events"}}
		for _, d := range devs {
	...
	// Insights
	if insights, err := s.GetRepoInsights(ctx, o, r); err == nil && len(insights) > 0 {
	...
	// Reputation
	if rep, err := s.GetReputationDistribution(ctx, o, r, nil, months); err == nil && rep != nil && len(rep.Labels) > 0 {
		rows := [][]string{{"range", "count"}}
